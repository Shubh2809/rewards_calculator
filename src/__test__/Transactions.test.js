...
describe('Transactions', () => {
  test('should render sorted transactions table with correct data', () => {
    ...
    expect(headers[3]).toHaveTextContent(PURCHASE_DATE);
    expect(headers[4]).toHaveTextContent(PRICE);
    expect(headers[5]).toHaveTextContent(REWARD_POINTS_HEADING);
    ...
    expectedOrder.forEach((data, index) => {
      ...
      expect(cells[1].textContent).toBe(data.customerName);
      expect(cells[2].textContent).toBe(data.product);
      expect(cells[3].textContent).toBe(new Date(data.purchaseDate).toLocaleDateString());
      expect(cells[4].textContent).toBe(data.amount.toFixed(2));
      expect(cells[5].textContent).toBe(Math.round(expectedPoints).toString());
    });
  });
});
